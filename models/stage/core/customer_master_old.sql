...
with cust_xref as (
        ...
        from (
            select to_char(cust_no) cust_no, 
                ...
                --ANALYTICS.EDW_CIS_US.DIM_PUB_CUSTOMER_XREF_US a
...
MD5(CONCAT(T1.SOURSYSTEM,'US01','US01','00','01',T1.TNCBPCUST)) AS CUSTOMER_MASTER_KEY,  -- Changed 11/13 by JDB
T1.SOURSYSTEM AS SOURSYSTEM,
-- T1.TNSALEORG AS SALES_ORG,
-- T1.TNSALEORG AS COMPANY_CODE,  -- Changed 11/13 by JDB
'US01' AS SALES_ORG,
'US01' AS COMPANY_CODE,  -- Changed 11/13 by JDB
'00' AS DIVISION,
'01' AS DISTR_CHANNEL,
T1.TNCBPCUST AS RESELLER_ID,
T1.TUCCUSTOR_46 AS RESELLER_ID_46,
--NVL(T1.TUCCUSTOR,TNCBPCUST) AS RESELLER_ID_COMBINED,
NVL(cust_xref.cust_no, NVL(T1.TUCCUSTOR_46, T1.TNCBPCUST)) AS RESELLER_ID_COMBINED,  -- Changed 11/13 by JDB
T1.TNCBPCUST AS RESELLER_ID_68 ,
NULL AS ACCOUNT_CLOSURE_FLAG,
NULL AS DELETION_FLAG_MAIN,
...
NULL AS ADDRESS,
T1.TNCCITY AS CITY,
T1.TNCCOUNTRY AS COUNTRY_KEY, -- Changed 11/13 by JDB
NULL AS CUST_CLASSIFICATION,
NULL AS COUNTY_CODE,  -- Changed 11/13 by JDB
...
SELECT  MD5(CONCAT(T1.SOURSYSTEM,'US01','US01',T2.TUCDIVISN,T2.TUCDISTRN,T1.TUCCUSTOR)) AS CUSTOMER_MASTER_KEY,
T1.SOURSYSTEM,
-- T2.TUCSALESG AS SALES_ORG,
-- T3."/BIC/TUCCOMPCE" AS COMPANY_CODE,
'US01' AS SALES_ORG,  -- Changed 11/13 by JDB
'US01' AS COMPANY_CODE,  -- Changed 11/13 by JDB
...
NVL(cust_xref.cust_no, T1.TUCCUSTOR) AS RESELLER_ID_COMBINED,  -- Changed 11/13 by JDB
...
, NULL as RESELLER_ID_68
, to_char(cust.IS_DISCONTINUED) as ACCOUNT_CLOSURE_FLAG
, to_char(cust.IS_DISCONTINUED) as DELETION_FLAG_MAIN
, NULL as LAST_PACE_FLAG
, NULL as PACE_FLAG_UPDATE
, NULL as LAST_PACE_CLUSTER
, NULL as LAST_CE_SEGMENT_TEXT
...
, NULL as COUNTY_CODE
, NULL as CUSTOMER_IS_CONSUMER
, to_char(cust.IS_DISCONTINUED) as DELETION_FLAG
, NULL as DUNS_NBR
, NULL as FAX_NBR
...
, to_char(xref.ENTRY_DATETIME) as CREATION_DATE
, NULL as BILL_BLOCK
, NULL as LEGAL_STATUS
, NULL as ASGMT_TO_HRCHY
, NULL as DELIVERY_WAVER
...
, NULL as ACCNT_NBR_DUN_RECIP
, NULL as CREDIT_CONTROL_AREA_CC
, to_char(cust.IS_DISCONTINUED) as DELETION_FLAG_CC
, NULL as EMPLOYEE
...
, NULL as ORDER_FULFILLMENT
, NULL as CREDIT_CONTROL_AREA_SS
, to_char(cust.IS_DISCONTINUED) as DELETION_FLAG_SS
, NULL as INCOTERMS_PART_1
, NULL as INCOTERMS_PART_2
, NULL as PLANT_SS
...
, NULL as ORDER_COMBINATION
, NULL as PARTIAL_DELIVERY_AT_
, to_char(creditinfo.last_edi_or_xml_date) as LAST_EDI_OR_XML
, NULL as DELIVERY_BLOCK_SS
, to_char(creditinfo.last_ec_order_date) as LAST_INTOUCH_ORDER
, to_char(creditinfo.last_purchase) as LAST_ORDER
