...
select 
md5(concat('CIS_CA',to_char(CUST.CUST_NO))) as CUSTOMER_MASTER_KEY
...
, SYSDATE() as UPDATE_DATE_UTC
from {{ source('ca_cdp_cis_ca','DIM_PUB_CUSTOMER_INFO_VIEW_CA') }} as cust   
--from ANALYTICS.EDW_CIS_CA.DIM_PUB_CUSTOMER_INFO_VIEW_CA  cust
left join {{ source('ca_cdp_cis_ca','DIM_PUB_SALES_HIERARCHY_BY_TERR_USER_ROLE_VIEW_CA') }} as terr
--left join ANALYTICS.EDW_CIS_CA.DIM_PUB_SALES_HIERARCHY_BY_TERR_USER_ROLE_VIEW_CA  terr
  on cust.sales_terr = to_char(terr.sales_terr)
  and user_role = 'Rep' 
  and is_primary = 'Y'
  and user_end_date is null  
left join {{ source('ca_cdp_cis_ca','DIM_PUB_CUSTOMER_XREF_CA') }} as xref
--left join ANALYTICS.EDW_CIS_CA.DIM_PUB_CUSTOMER_XREF_CA xref
  on cust.cust_no = xref.cust_no
  and xref_type = 'MASTER_SUB'
  and active = 'Y'  
